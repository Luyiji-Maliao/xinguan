...
<head>
    ...
    <meta http-equiv="description" content="客户信息">
...
<br>
    <div class="form-group">
        <h1  style="border:0px solid #fff;margin-left:20px;font-size:50px;height:80px;background-color: #fff;"><span class="glyphicon glyphicon-chevron-left" onclick="back()"></span>客户信息</h1>
        <h1 style="width: 241px; height: 100px; position: absolute;right:360px;top:10px;line-height: 100px;font-size: 40px;" onclick="getall()">全部</h1>
        <div class='datatimepicker'>
            <div class="form-group">
                <!--指定 date标记-->
                <div class='input-group date' id='datetimepicker2'>
                    <input type='date' class="form-control" id="date"  value=""  onchange="search()" />
                </div>
            </div>
        </div>
        <select name="select" id="select" class="form-control" onchange="select()">
            <option value="全部采样点">全部采样点</option>
        </select>
...
var isflag=true;
    ...
    */
    function init() {
        ...
        hf=hf.substring(hf.indexOf("cooperationstr=")+15);
        if(hf.indexOf("&")>-1){
            hf=hf.substring(0,hf.indexOf("&"));
        }
        var caiyangdian=$("#select").val();
        //显示等待条
        $("#loadingModal").modal('show');
        $.post({
            url:"sign!getbyma.action",
            data:{"shicaiyangdian":caiyangdianname,"name":valuea,"start":start,"limit":limit,"samplebindtime":samplebindtime,"cooperationstr":hf},
            success:function (data){
                ...
                if(isflag){
                    $("#date").attr("min",data.mindate);
                    if(data.maxdate && data.maxdate.indexOf(" ")>-1){
                        data.maxdate=data.maxdate.substr(0,data.maxdate.indexOf(" "));
                    }
                    $("#date").attr("max",data.maxdate);
                    $("#select").html("<option value=\"全部采样点\">全部采样点</option>");
                    for (var i=0;i<data.dilist.length;i++){
                        var ht="<option value='"+data.dilist[i]+"'>"+data.dilist[i]+"</option>";
                        $("#select").append(ht);
                    }
                    $("#select").val(caiyangdian);
                    isflag=false;
                }
                ...
                for (var i=0;i<data.length;i++){
                    if(data[i].shicaiyangdian==null){
                        data[i].shicaiyangdian=data[i].caiyangdidian;
                    }
                    if(data[i].samplebindtime==null){
                        data[i].samplebindtime=data[i].yuyuedate+" "+data[i].yuyuetime
                    }
