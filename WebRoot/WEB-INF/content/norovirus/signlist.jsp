...
<head>
    ...
    <meta http-equiv="description" content="待签到列表">
        ...
        <div class='datatimepicker'>
            <div class="form-group">
                ...
                <div class='input-group date' id='datetimepicker2'>
                    <input type='date' class="form-control" id="date"  onchange="search1()" />
                </div>
            </div>
        </div>
...
<div id="app" >
    ...
    <%--<van-button type="primary" onclick="search1()" id="sear" style="position: absolute;right:20px;bottom:10px;display:none;" >搜索</van-button>--%>
...
<script>
    ...
    */
    function init() {
        ...
        $.post({
            url:"sign!list.action",
            data:{"caiyangdidian":$("#select").val(),"name":valuea,"start":start,"limit":limit,"samplebindtime":$("#date").val()},
            success:function (data){
                ...
                for (var i=0;i<data.length;i++){
                    if(data[i].caiyangdidian.toString().indexOf("(")>=0){
                        data[i].caiyangdidian=data[i].caiyangdidian.toString().substring(0,data[i].caiyangdidian.toString().indexOf("("))
                    }
                    var html="<div class=\"van-cell\"><div class=\"van-cell__title\"><span>"+data[i].name+"</span><div class=\"van-cell__label\">"+data[i].caiyangdidian+"\t"+data[i].yuyuetime+"</div></div><div class=\"van-cell__value\"><span><a href='tel:"+data[i].phone+"' class='tela'>"+data[i].phone+"</a><br/>"+(data[i].yuyuenum==null?'':data[i].yuyuenum)+"</span></div></div>";
                    $(".van-cell-group").append(html);
                }
            },error:function () {
                alert("错误，请联系管理员！");
