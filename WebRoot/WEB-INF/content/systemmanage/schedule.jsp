            ...
            var portal = Ext.create('Ext.panel.Panel',{  
                            ...
                            icon:'img/newsave.png',
                             handler :savePosition  
                           	...
                           	text:'添加待办类型',
                           	 icon:'img/add.png',
                           	handler:function(){
                           	Ext.getCmp("saveorupdates").setValue("添加");
                           ...
                           }, 
						{
							...
							handler:function(){
								...
								this.ownerCt.ownerCt.items.get(0).items.each(function(item,index,length){
									item.items.each(function(i){
										if(i.title=="其他"){
											gridid="show_data_grid"+i.id;
											ss_combox.setValue(i.id);// 待办类别 
											return false;
										}
									})
								}); 
								...
								Ext.getCmp("priorityLevel").hide();//任务优先级
		...
		dataIndex : 'scheduleSenderDate'
			...
			dataIndex : 'scheduleSender'
				...
				tbar : [ {
					...
					gridid=this.ownerCt.ownerCt.getId();
					Ext.getCmp("saveorupdate").setValue("添加");
					Ext.getCmp("subbut").setText("添加");
					//设置只读以及背景色 s
					...
					Ext.getCmp("schedulebh").hide();//驳回按钮隐藏 
					ss_combox.hide();//隐藏任务类别
					ss_combox.setValue(gridid.substring(14,gridid.length));// 待办类别 
					crud_data_win.show();
					}
				},
				{
					...
					gridids=this.ownerCt.ownerCt.getId();
					gridid=this.ownerCt.ownerCt.getId();
					othersgrid=this.ownerCt.ownerCt.ownerCt.ownerCt.ownerCt;
					
					Ext.getCmp("saveorupdate").setValue("修改");
					var record =Ext.getCmp(gridids).getSelectionModel().getSelection();
					if(record.length){
				        ...
				        crud_data_win.show();
				        sortcom=record[0].get("scheduleSortId");
				      ...
				      Ext.getCmp('show_data_grid'+p[j].id).getStore().getProxy().extraParams.filter_EQS_scheduleStatus="1";
