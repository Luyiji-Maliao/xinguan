...
Ext.define('Ext.ux.uploadPanel.UploadPanel',{
	...
	columns : [
        ...
        {text: '状态', width: 120,dataIndex: 'status',renderer:function(v){
			...
			if(v==-1){
				...
				status =  "<div><img src='js/ext4xupload/uploadPanel/icons/ok.png'/></div>";
			}else if(v==-5){
				status =  "停止上传";
			}		
			return status;
		}},
        {
            ...
            width:50,
            items: [{
                icon: 'js/ext4xupload/uploadPanel/icons/delete.gif',
                tooltip: 'Remove',
	...
	upload_start_handler : function(file){
		...
		this.addPostParam("newFileName",encodeURIComponent(rec.get('fileName')));
	},
	...
	upload_success_handler : function(file, serverData, responseReceived){
		if(eval('(' + serverData + ')').result){
			Ext.MessageBox.show({
		        title: '提示',
		        msg: eval('(' + serverData + ')').result,
		        buttons: Ext.MessageBox.OK,
		        //fn: DemoFunc.evtAfterClick,
		        icon: Ext.MessageBox.INFO
		    });
			 //Ext.example.msg("提示",eval('(' + serverData + ')').result);
			 var me = this.settings.custom_settings.scope_handler;		
				var rec = me.store.getById(file.id); 
